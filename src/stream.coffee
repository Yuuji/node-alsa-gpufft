...
Pcm = require('../build/Release/alsa-gpufft').Pcm

class Capture extends stream.Readable
  constructor: (@device = 'default', channels = 2, rate = 44100, format = alsa.FORMAT_S16_LE, access = alsa.ACCESS_RW_INTERLEAVED, latency = 500, fft = false) ->
    stream.Readable.call this
    @pcm = new Pcm channels, rate, format, access, (latency * 1000), fft
